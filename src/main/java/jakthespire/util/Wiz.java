...
import jakthespire.actions.TimedVFXAction;
...
public class Wiz {
    ...
    public static void removeFromSelf(String po) {
        atb(new RemoveSpecificPowerAction(AbstractDungeon.player, AbstractDungeon.player, po));
    }

    public static void removeFromSelfTop(String po) {
        att(new RemoveSpecificPowerAction(AbstractDungeon.player, AbstractDungeon.player, po));
    }
    ...
    public static void hpDmg(AbstractCreature m, int amount, AbstractGameAction.AttackEffect AtkFX) {
        atb(new DamageAction(m, new DamageInfo(AbstractDungeon.player, amount, DamageInfo.DamageType.HP_LOSS), AtkFX));
    }

    public static void hpDmg(AbstractCreature m, int amount) {
        hpDmg(m, amount, AbstractGameAction.AttackEffect.NONE);
    }
    ...
    public static String cardPlayed(int a) {
        if (AbstractDungeon.actionManager.cardsPlayedThisTurn.size() >= a) {
            return AbstractDungeon.actionManager.cardsPlayedThisTurn.get(AbstractDungeon.actionManager.cardsPlayedThisTurn.size() - a).cardID;
        }
        return "null";
    }

    public static boolean cardPlayedUpped(int a) {
        if (AbstractDungeon.actionManager.cardsPlayedThisTurn.size() >= a) {
            return AbstractDungeon.actionManager.cardsPlayedThisTurn.get(AbstractDungeon.actionManager.cardsPlayedThisTurn.size() - a).upgraded;
        }
        return false;
    }

    public static void queueCard(AbstractCard c, AbstractMonster m) {
        atb(new NewQueueCardAction(c, m, true, true));
    }

    public static void queueCardTop(AbstractCard c, AbstractMonster m) {
        att(new NewQueueCardAction(c, m, true, true));
    }

    public static long cardCount(String c) {
        return AbstractDungeon.player.masterDeck.group.stream().filter(a -> a.cardID.equals(c)).count();
    }
}
