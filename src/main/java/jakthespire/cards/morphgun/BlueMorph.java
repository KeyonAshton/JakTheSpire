...
public class BlueMorph extends AbstractEasyCard {
    public final static String ID = makeID("BlueMorph");
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
        ...
        if (pwrAmt(p, BlueAmmo.getID) < ammoMax) {
            applyToSelf(new BlueAmmo(p, magicNumber));
            while (pwrAmt(p, BlueAmmo.getID) < ammoMax){
                applyToSelf(new BlueAmmo(p, -1));
            }
        }
        if (timesUpgraded > 0) {
            applyToSelf(new ArcWielder(p, 1));
        } else {
            applyToSelf(new VulcanFury(p, 1));
        }
    }
