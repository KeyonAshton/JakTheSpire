...
import static jakthespire.util.Wiz.cardPlayed;

public class DarkEcoBox extends AbstractEasyCard implements SpawnModificationCard {
    public final static String ID = makeID("DarkEcoBox");
    ...
    public void triggerWhenDrawn() {
        ...
        for(int i = 1; cardPlayed(i).equals(PlasmiteBomb.ID) || cardPlayed(i).equals(TheBomb.ID) || cardPlayed(i).equals(DarkEcoBox.ID) || cardPlayed(i).equals(DarkEcoCrate.ID); i++) {
            if (cardPlayed(i).equals(DarkEcoBox.ID)) {
                damage += magicNumber;
                secondMagic++;
            }
        }
    ...
    public void triggerOnOtherCardPlayed(AbstractCard c) {
        damage = baseDamage;
        secondMagic = 0;
        if(cardPlayed(1).equals(DarkEcoBox.ID)) {
            damage += magicNumber;
            secondMagic++;
        } else if (!cardPlayed(1).equals(TheBomb.ID) || !cardPlayed(1).equals(PlasmiteBomb.ID) || !cardPlayed(1).equals(DarkEcoCrate.ID)) {
            secondMagic = 0;
            damage = baseDamage;
        }
        rawDescription = cardStrings.DESCRIPTION + cardStrings.EXTENDED_DESCRIPTION[0] + secondMagic + cardStrings.EXTENDED_DESCRIPTION[1];
